...
\section{Projektivne reprezentacije i orbitalni moment impulsa}
...
Važniji je slučaj $j=1/2$ reprezentacije. Ona je dvodimenzionalna i
...
vjernu reprezentaciju grupe \SU{2}. No, je li ona i reprezentacija
grupe rotacija \SO{3}?
...
reprezentacije.
No po definiciji, reprezentacija $\Gamma=\{D(\phi)\}$ grupe G mora zadovoljavati
općeniti uvjet homomorfizma s grupom
\begin{displaymath}
...
Ta promjena faze
je i eksplicitno dokazana eksperimentima s fermionskim sustavima.
...
\label{eq:Lorb}
\end{equation}
gdje \suAlg{2} komutacijske relacije slijede izravno iz
...
\end{equation}
i odgovarajući momenti impulsa tj. ireduciblne
reprezentacije se označavaju s $l$, a ne $j$.
...
ireduciblne na \emph{konačnodimenzionalnim} prostorima razapetim
funkcijama $Y^{(l)}_{m}(\theta, \phi)$ za $m \in \{-l, -l+1, \ldots, l\}$.
Dakle potpuno smo u domeni rezultata prošlog odjeljka i ostaje
samo pitanje što je s polucjelobrojnim reprezentacijama.
...
No sve fizikalne posljedice su, kako smo maločas diskutirali u
kontekstu projektivnih reprezentacija, neosjetljive na fazu valne funkcije
