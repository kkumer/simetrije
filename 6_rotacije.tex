...
\end{equation}
Kompozicija dvaju rotacija u ravnini za kutove $\phi_1$ i $\phi_2$ je rotacija za
...
\[\ket{j, m},\quad m\in\{-j, -j+1, \ldots, j-1, j\}\,,\] 
...
\begin{equation}
    ...
    \label{eq:WignerD}
\end{equation}
nazivaju se \emph{Wignerove D-funkcije} i formiraju tzv. \emph{Wignerovu D-matricu}.
...
prihvatimo da je zapravo \SU{2} "istinska" grupa rotacija i prava simetrija prirode
...
Izložit ćemo sada argument J. Schwingera koji objašnjava zašto $l$ ne može biti
...
operatore imamo sada izraz za bezdimenzionalni operator momenta impulsa
oko $z$-osi
\begin{equation}
...
(uz $D(R) \equiv D(\hat{\vec{n}},\phi)$)
\begin{equation}
    r_i \longrightarrow D(R) r_i D(R)^{-1} = R_{ij} r_j \;.
\end{equation}
...
\begin{align}
    D(R) \Big( T^{(k)}_q\ket{j,m} \Big)& =
        D(R)  T^{(k)}_q D(R)^{-1} D(R) \ket{j,m}  \\
...
atoma \SO{3}$\times$\SO{3}\footnote{Algebra grupe
\SO{3}$\times$\SO{3} izomorfna je algebri grupe \SO{4} pa otud naslov ovog odjeljka. 
